...
//
// Vendored verbatim from PITVIPER's own internal/vterm (DUNG NORTHSTAR.md
// Phase 1: "PTY + vterm, hand-ported or vendored from PITVIPER's own real,
// already-working internal/vterm/internal/pty -- the terminal domain
// doesn't wait on BURROW's own emitter work, since it's Go-hosted, not
// PARENA-compiled"). PITVIPER's own Go implementation is untouched by this
// copy -- DUNG is a new, separate fork per its own NORTHSTAR.md, not a
// rewrite-in-place. Package name/API kept identical on purpose so future
// fixes can be diffed/ported both ways without a rename getting in the way.
package vterm
