...
same-day: `go build`/`go vet`/`go test` clean, `bazel build //...` clean, verified running for
real under Xvfb (real bash, real colored PTY output rendered through vterm→font→SDL2) both ways.
First real PARENA mod wired in: `internal/burrowgen` (generated via `BURROW`'s own new Phase 6 Go
emission target) supplies real decision logic to `cmd/dung/main.go` directly, no cgo/FFI. See
`NORTHSTAR.md` for the full proof and honest open items.
